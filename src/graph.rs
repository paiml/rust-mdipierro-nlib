...
impl Graph {
    ...
    pub fn to_aprender(&self) -> AprGraph {
        ...
        if edges.is_empty() {
            ...
            AprGraph::from_edges(&(0..self.n).map(|i| (i, i)).collect::<Vec<_>>(), true)
        } else {
...
mod tests {
    ...
    #[test]
    fn dfs_all_reachable() {
        assert_eq!(dfs(&five_node(), 0).len(), 5);
    }
    #[test]
    fn dfs_disconnected() {
        assert_eq!(dfs(&Graph::new(3), 0), vec![0]);
    }
    #[test]
    fn kruskal_tri() {
    ...
    #[test]
    fn kruskal_single() {
        assert!(kruskal_mst(&Graph::new(1)).is_empty());
    }
    #[test]
    fn kruskal_edge_count() {
        assert_eq!(kruskal_mst(&five_node()).len(), 4);
    }
    #[test]
    fn to_aprender_conversion() {
        let g = triangle();
